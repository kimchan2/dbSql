...
아래 두개의 sql을 서로 다른 SQL로 인식
SELECT * FROM emp;
Select * FROM emp;

특정 직원의 정보를 조회하고 싶은데 : 사번을 이용해서
Select /* 202004_B */ *
FROM emp
WHERE empno = :empno;
...
SYNONYM : 동의어, 객체의 별칭을 생성해서 별칭을 통해 원본 객체를 사용
문법 : CREATE SYNONYM 시노님 이름 FOR 대상 오브젝트;

chan.v_emp ==> v_emp 시노님으로 생성

CREATE SYNONYM v_emp FOR chan.v_emp;
...
emp 테이블에는 존재하고 emp_test 테이블에도 존재하는 직원을 이름 변경
...
(사용자가 9999번, james로 사원을 등록하거나, 업데이트 하고싶을 때)
...
-- dual 테이블은 문법상 어떤 테이블을 넣어야하는데 넣을게 없어서 넣음
-- dual 쓰는 형태가 더 자주 쓰임
MERGE INTO emp_test
USING dual
...
GROUP BY 를 사용하는 두 개의 SQL로 나눠서 하나로 합치는(UNION ==> UNION ALL) 작업을 진행
