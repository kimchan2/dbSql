...
SMITH 도는 ALLEN이 속하는 부서의 조직원 정보를 조회

행이 여러개고, 컬럼은 하나다 
==> 서브쿼리에서 사용가능한 연산자 IN(많이씀), ANY, ALL (두개는 빈도가 낮음)
IN : 서브쿼리의 결과값중 동일한 값이 있을 때 TRUE
WHERE 컬럼|표현식 IN (서브쿼리)

ANY : 연산자를 만족하는 값이 하나라도 있을 때 TRUE
    WHERE 칼럼|표현식 연산자 ANY (서브쿼리)

ALL : 서브쿼리의 모든 값이 연산자를 만족할 때 TRUE
    WHERE 칼럼|표현식 연산자 ALL (서브쿼리)
...
ANY, ALL
SMITH나 WARD 두 사원의 급여중 아무 값보다 작은 급여를 받는 직원 조회
==> sal < 1250
SELECT *
FROM emp
...
SMITH나 WARD 두 사원의 급여보다 큰 급여를 받는 직원 조회                 
==> sal > 1250
SELECT *
FROM emp
...
PAIREWISE 적용 (우의 쿼리보다 결과가 한 건 적다)
SELECT *
FROM emp
...
생각해볼 문제, 위의 쿼리는 상호 연관 서브쿼리인가? 비상호 인가? ==> 비상호 연관
...
WHERE sal > (SELECT AVG(sal)
             FROM emp
             WHERE deptno = e.deptno); ==> 상호 연관
             
             
아우터 조인 => 테이블의 컬럼은 조인에 실패하더라도 조회가 된다
oracle 9i 이전 까지는 기준이 되는 테이블 부터 읽는다
==> 10g 이후 부터는 성능상 유리한 테이블 부터 읽는다
...
동일한 부서번호가 서브쿼리에서 조회되지 않도록 제거 할려고 그룹 연산을 한경우 (답은 맞음)                      
SELECT *
FROM dept
