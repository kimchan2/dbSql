...
테이블 생성시 제약조건을 특별히 생성하지 않았음

테이블 변경을 통한 PRIMARY KEY 추가

문법 : ALTER TABLE 테이블명 ADD CONSTRAINT 제약조건명 제약조건타입 (적용할 컬럼[,....]);
제약조건 타입 : PRIMARY KEY, UNIQUE, FOREIGN KEY, CHECK, (NOT NULL)
...
제약조건 활성화 비활성화
테이블에 설정된 제약조건을 삭제하는 것이 아니라 잠시 기능을 꺼두는 설정
...
fk_emp_test_dept_test 제약조건이 비활성화되어 있기 때문에
emp_test 테이블에는 99번 부서 이외의 값이 입력가능 하다

dept_test 테이블에 88번 부서가 없지만 아래 쿼리는 정상적으로 실행
INSERT INTO emp_test VALUES (9999, 'brown', 88);

현재 상황 : empt_test 테이블에 dept_test 테이블에 존재하지 않는 88번 부서를 사용하고 있는 상황
...
USER_CONSTRAINT : 제약조건 정보(MASTER);
...
컬럼 주석 생성
문법 : COMMENT ON COLUMN 테이블명.컬럼명 IS '주석';
...
v_emp view는 chan계정 소유
HR계정에게 인사 시스템 개발을 위해서 EMP테이블이 아닌 SAL, COMM 조회가 제한된
v_emp view를 조회할 수 있도록 권한을 부여

권한 부여전 hr 계정에서 v_emp 조회 실패
SELECT *
FROM chan.v_emp;

chan계정에서 hr계정으로 v_emp view를 조회할 수 있는 권한 부여
GRANT SELECT ON v_emp TO HR;

v_emp view 권한을 hr 계정에 부여한 이후 조회 테스트 성공
SELECT *
FROM chan.v_emp;
...
ex : 사번, 학번, 게시글 번호
     사번, 학번 : 체계
     사번 : 15101001 - 회사 설립년차 15, 10월 10일, 해날 날짜에 첫번째 입사한 사람 01
     학번 : 
...
2. JAVA 개발자 UUID 클래스를 활용, 별도의 라이브러리 활용(유료) ==> 금융권, 보험, 카드
   ==>
