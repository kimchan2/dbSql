...
연말정산 : 소득의 25%를 신용카드 + 현금영수증 + 체크카드로 소비를
         ...
         신용카드는 공제 : 20%, 현금영수증은 30%, 체크카드 25% 공제하는데
         단, 공제 금액이 300만원을 넘을 수 없고
         단, 대중교통에 대해 추가적으로 100만원을 공제 받을 수 있고
         단, 전통시장에서 사용한 금액에 대해서는 추가적으로 100만원을 공제받을 수 있다;
...
PL/SQL에도 존재
SET SERVEROUTPUT ON; 로그를 콘솔창에 출력가능하게끔 하는 설정, 안하면 출력이 안됨
...
날짜를 입력받아 ==> 그회사의 영업일기준으로 5일 뒤의 날짜를 리턴하는 함수 => 회사만의 특이한 로직이 필요한 경우 함수로 만들어야댐 ㅋ



PROCEDURE : 이름이 있는 PL/SQL 블록, 리턴값이 없다
            로직에 따라 처리후 데이터를 다른 테이블에 입력하는 등의
            비지니스 로직을 처리 할 때 사용
...
복합변수
조회결과를 컬럼당 하나의 변수에 담는 작업이 번거롭다 ==> 복합 변수를 사용하여 불편함을 해소
...
1. %ROWTYPE
익명블럭 dept 테이블의 10번 부서정보를 조회하여 %ROWTYPE으로 선언한 변수에 결과값을 저장하고
...
DECLARE
    ...
    /*새롭게 만든 타입으로 변수를 선언 (class 만들고 인스턴스 생성하는 것과 비슷)*/
    v_dept_rec dept_rec;
BEGIN
...
3. TABLE TYPE : 여러행을 저장할 수 있는 타입
문법 : TYPE 타입명 IS TABLE OF 행 타입 INDEX BY 인덱스에 대한타입;
...
dept테이블의 행 정보를 저장할 수 있는 테이블 TYPE
    ...
    PL/SQL에서는 두가지로 타입을 지원 : 정수(BINARY_INTEGER), 문자열(VARCHAR(2))
    intArray["첫번째"] = 50;
    System.out.println(intArray["첫번째"]); 문자열 인덱스는 요런 느낌
