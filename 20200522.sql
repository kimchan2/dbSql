...
행전체를 저장할 수 있는 변수(Map,Class)
여러행을 저장할 수 있는 변수(List<Map>, List<Class>)
...
REVERSE 옵션을 사용하면 종료값부터 값을 1씩 줄여 나가며 인덱스변수가 시작값이 될 때까지 실행
...
어렵지만 중요한 것
CURSOR : SELECT 문에 의해 추출된 데이터를 가리키는 포인터
...
문법
FOR 레코드이름(행정보를 저장) IN 커서이름 LOOP
    레코드이름.컬럼명으로 컬럼정보 접근
END FOR;
